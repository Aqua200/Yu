...
const handler = async (m, { conn, text, command }) => { try { if (!text.trim()) { return conn.reply(m.chat, ⚔️ ingresa el nombre de la música a descargar., m); }
...
📌 *🎬  Título : » ${title} 👀 *💯 Vistas: » ${vistas} ⏱️ *⏳ Duración : » ${timestamp} 📅 *🕒 Publicado : » ${ago} 🔗 *🌐 URL : » ${url}`;

const thumb = (await conn.getFile(thumbnail))?.data;

await conn.reply(m.chat, infoMessage, m);

if (['play', 'play5', 'yta', 'ytmp3'].includes(command)) {
...
handler.command = handler.help = ['play', 'play2', 'play5', 'ytmp3', 'yta', 'ytmp4', 'ytv']; handler.tags = ['downloader'];
